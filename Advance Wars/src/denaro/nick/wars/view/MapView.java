...
public class MapView extends GameView2D implements CursorListener
{
	...
	{
		return(new Point(view.x*Main.TILESIZE,view.y*Main.TILESIZE+Main.TILESIZE));
	}
	...
	public void cursorMoved(Point cursor)
	{
		...
		if((cursor.x-view.x+1)*Main.TILESIZE<0)
		{
			view.x--;
			viewMoved();
		}
		if((cursor.y-view.y+1)*Main.TILESIZE>height()-1)
		{
			view.y++;
			viewMoved();
		}
		...
		//System.out.println("Cursor: "+cursor+"|View: "+view);
	}
	...
	public void drawTerrain(Map map, Graphics2D g)
	{
		for(int a=0;a<map.height();a++)
		{
			for(int i=0;i<map.width();i++)
			{
				...
				int direction=map.terrainDirections().direction(i,a);
				if(Main.currentMode instanceof Battle)
					if(((Battle)Main.currentMode).weather().fog())
						if(((Battle)Main.currentMode).fog(i, a))
							g.drawImage(addFog((BufferedImage)map.terrain(i,a).image(direction)),i*Main.TILESIZE-sprite.anchor().x,a*Main.TILESIZE-sprite.anchor().y,null);
						else//no fog, so just draw
							g.drawImage((BufferedImage)map.terrain(i,a).image(direction),i*Main.TILESIZE-sprite.anchor().x,a*Main.TILESIZE-sprite.anchor().y,null);
					else
						g.drawImage((BufferedImage)map.terrain(i,a).image(direction),i*Main.TILESIZE-sprite.anchor().x,a*Main.TILESIZE-sprite.anchor().y,null);
			...
			for(int i=0;i<map.width();i++)
			{
				...
				if((Main.currentMode instanceof Battle==false)||((!((Battle)Main.currentMode).weather().fog())||(!((Battle)Main.currentMode).fog(x,y))))
				{
					if(unit!=null)
					{
						...
						boolean show=false;
						if(unit.isHidden())
							nextTo=((Battle)Main.currentMode).enemiesNextToUnit(new Point(i,a));
						for(Point p:nextTo)
							if(Team.sameTeam(((Battle)Main.currentMode).map().unit(p.x,p.y).team(),((Battle)Main.currentMode).myTeam()))
								show=true;
						if((!unit.isHidden()||show)||Team.sameTeam(unit.team(),((Battle)Main.currentMode).myTeam()))
						{
							if(!unit.enabled())
							{
	...
	public void drawInfo(Map map, Graphics2D g)
	{
		...
		if(unit!=null)
		{
			...
			boolean show=false;
			if(unit.isHidden())
				nextTo=((Battle)Main.currentMode).enemiesNextToUnit(new Point(cursor.x,cursor.y));
			for(Point p:nextTo)
				if(Team.sameTeam(((Battle)Main.currentMode).map().unit(p.x,p.y).team(),((Battle)Main.currentMode).myTeam()))
					show=true;
			if((!unit.isHidden()||show)||Team.sameTeam(unit.team(),((Battle)Main.currentMode).myTeam()))
